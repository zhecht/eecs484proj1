...
FB_User U, Education E, Location LC, Location LH WHERE
LC.LOCATION_ID IN (SELECT CURRENT_LOCATION_ID FROM UserLivesInLocation UR WHERE UR.USER_ID = U.USER_ID) AND
LH.LOCATION_ID IN (SELECT HOMETOWN_LOCATION_ID FROM UserLivesInLocation UR WHERE UR.USER_ID = U.USER_ID) AND
E.USER_ID = U.USER_ID
;

INSERT INTO VIEW_USER_INFORMATION (USER_ID, FIRST_NAME, LAST_NAME, YEAR_OF_BIRTH, MONTH_OF_BIRTH, DAY_OF_BIRTH,
GENDER) SELECT USER_ID, FIRST_NAME, LAST_NAME, YEAR_OF_BIRTH, MONTH_OF_BIRTH, DAY_OF_BIRTH, GENDER FROM FB_User;

UPDATE VIEW_USER_INFORMATION, Education
SET	VIEW_USER_INFORMATION.INSTITUTION_NAME = Education.INSTITUTION_NAME,
 	VIEW_USER_INFORMATION.PROGRAM_YEAR = Education.PROGRAM_YEAR,
 	VIEW_USER_INFORMATION.PROGRAM_CONCENTRATION = Education.PROGRAM_CONCENTRATION,
 	VIEW_USER_INFORMATION.PROGRAM_DEGREE = Education.PROGRAM_DEGREE
WHERE 
VIEW_USER_INFORMATION.USER_ID = Education.USER_ID;

UPDATE VIEW_USER_INFORMATION, Location
SET VIEW_USER_INFORMATION.HOMETOWN_CITY = Location.CITY,
	VIEW_USER_INFORMATION.HOMETOWN_STATE = Location.STATE,
	VIEW_USER_INFORMATION.HOMETOWN_COUNTRY = Location.COUNTRY
WHERE
Location.LOCATION_ID = (SELECT HOMETOWN_LOCATION_ID FROM UserLivesInLocation WHERE USER_ID = VIEW_USER_INFORMATION.USER_ID)

UPDATE VIEW_USER_INFORMATION, Location
SET VIEW_USER_INFORMATION.CURRENT_CITY = Location.CITY,
	VIEW_USER_INFORMATION.CURRENT_STATE = Location.STATE,
	VIEW_USER_INFORMATION.CURRENT_COUNTRY = Location.COUNTRY
WHERE
Location.LOCATION_ID = (SELECT HOMETOWN_LOCATION_ID FROM UserLivesInLocation WHERE USER_ID = VIEW_USER_INFORMATION.USER_ID)
